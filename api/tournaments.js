...
function normalizedDraft(body, id) {
  ...
  return {
    ...
    maxPlayers,
    pointsToWin: Math.max(0, Math.min(99, Math.round(number(body.pointsToWin, 15)))),
    winBy: Math.max(0, Math.min(10, Math.round(number(body.winBy, 1)))),
    settings: {
      ...settingsInput,
      ...
      scoringFormat: settingsInput.scoringFormat || body.scoringFormat || settingsInput.ruleConfiguration?.scoringFormat || settingsInput.ruleConfiguration?.rules?.scoringFormat || 'points',
      ruleConfiguration: settingsInput.ruleConfiguration || buildScoringConfiguration({ sport: settingsInput.sport || body.sport || 'pickleball', mode: settingsInput.scoringMode || body.scoringMode || 'official', overrides: settingsInput.ruleOverrides || {} }),
      numberOfRounds,
